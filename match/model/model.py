...
class MatchModel:
    ...
    def compile(self, target, out_path):
        ...
        if self.relay_mod is None:
            if self.is_model_dynamic:
                ...
                self.relay_mod, self.relay_params, self.other_models = get_dyn_relay_from(
                    self.model_type,
                    self.filename,
        ...
        if len(self.other_models) > 0:
            for model_name, model in self.other_models.items():
                ...
                comp = EXCUTOR_COMPILER_CLS[get_executor()](
                    model[0], model[1], target=target, build_dir=model_path, mod_name=model_name
                )
                comp.tvm_compile(fusion=True)
                MatchModel.save_model_logs()
                MatchModel.gen_model_runtime_and_move_model(
                    target=target,
                    out_path=out_path,
        ...
        if self.golden_cpu_model:
            ...
            comp = EXCUTOR_COMPILER_CLS[get_executor()](
                self.relay_mod, self.relay_params, target=target, build_dir=model_path, mod_name=self.model_name + "_golden_cpu"
            )
            comp.tvm_compile(fusion=True)
            MatchModel.save_model_logs()
            MatchModel.gen_model_runtime_and_move_model(
                target=target,
                out_path=out_path,
                model_name=self.model_name + "_golden_cpu",
                executor=get_executor(),
                match_inputs=match_inputs,
                debug=self.debug,
            )
        ...
        comp = self.compilation_cls(
            self.relay_mod, self.relay_params, target=target, build_dir=model_path, mod_name=self.model_name
        )
        comp.tvm_compile(fusion=True)
        MatchModel.save_model_logs()
        MatchModel.gen_model_runtime_and_move_model(
    ...
    def model_runtime(self, target=None, out_path: str = "/build"):
        ...
        temp_args = {
            ...
            "inputs": match_inputs,
            "all_model_names": [self.model_name]
            + ([] if not self.golden_cpu_model else [self.model_name + "_golden_cpu"])
        ...
        if not Path(abs_out_path + "/runtime").is_dir():
            subprocess.getoutput(
                f"unzip {Path(os.path.dirname(__file__) + '/tvm_runtime.zip').absolute()} -d {Path(abs_out_path + '/')}"
            )
        ...
        with open(abs_out_path + f"/src/{self.model_name}/runtime.c", "w") as run_file:
            run_file.write(
                Template(filename=os.path.dirname(__file__) + "/../libs/c/mako/match/src/runtime.c").render(**temp_args)
            )
        with open(abs_out_path + f"/include/{self.model_name}/runtime.h", "w") as run_file:
            run_file.write(
                Template(filename=os.path.dirname(__file__) + "/../libs/c/mako/match/include/runtime.h").render(**temp_args)
            )
        ...
        if executor == "graph":
            ...
            try:
                ...
                with open(f"{build_dir}/codegen/host/src/{model_name}_params_data.c", "w") as run_file:
                    filename = os.path.dirname(__file__) + "/../libs/c/mako/match/src/params_data.c"
                    content = Template(filename=filename).render(**graph_runtime_template_data)
                    run_file.write(format_c_code(content))
                with open(f"{build_dir}/codegen/host/include/{model_name}_params_data.h", "w") as run_file:
                    filename = os.path.dirname(__file__) + "/../libs/c/mako/match/include/params_data.h"
                    content = Template(filename=filename).render(**graph_runtime_template_data)
                    run_file.write(format_c_code(content))
            ...
            for exec_module in target.exec_modules:
                if exec_module.separate_build:
                    ...
                    em_runtime_template_data["nodes"] = [
                        ...
                        if node.schedule and node.schedule.exec_module.name == exec_module.name
                    ]
    ...
    def get_match_inputs_and_outputs(self):
        ...
        if default_inputs is None:
            ...
            np.random.seed(0)
            default_inputs = [
                get_random_np_array(dtype=inp_.type_annotation.dtype, shape=tuple([int(v) for v in inp_.type_annotation.shape]))
        ...
        match_outputs = {
            f"output{idx if len(relay_out_types) > 1 else ''}": {
                ...
                "is_copy_of": ""
                if not isinstance(func.body, relay.Tuple)
                or not any([func.body[idx] == func_out for func_out in list(func.body)[:idx]])
                else f"output{[func_out for func_out in range(idx) if func.body[idx] == func.body[func_out]][0]}",
                "shape": [int(sh) for sh in out.shape],
                "bytes": int(prod(out.shape) * np.dtype(out.dtype).itemsize),
                "associated_input": ""
                if not isinstance(func.body, relay.Tuple) or not isinstance(func.body[idx], relay.Var)
                else to_c_variable_name(func.body[idx].name_hint),
